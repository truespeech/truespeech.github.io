...
async function main() {
  ...
  try {
    ...
    const jsYaml: any = await import(jsYamlUrl);
    const yamlText = await fetch(IMPORTS.semanticModel).then((r) =>
      r.text()
    );
...
async function handleOsiCommand(
  input: string,
  runtime: any,
  osiRepl: ReturnType<typeof createRepl>,
  ...
  switch (parsed.type) {
    ...
    case "metrics": {
      ...
      if (metrics.length === 0) {
        ...
        const lines = metrics.map((m: { name: string; description?: string }) => {
      ...
      try {
        ...
        if (dims.length === 0) {
          ...
          const lines = dims.map(
            (d: { name: string; isTime: boolean; description?: string }) => {
