...
export async function makeSeededTs(deps, seedStatements = []) {
    const lexicon = new MemoryLexicon();
    const ts = new deps.tsModule.TrueSpeech({
...
export async function createFrozenCell(source, deps, seedStatements = []) {
...
export async function createStandaloneCell(config, deps) {
...
function renderCannedItem(item, runWith, loadInto) {
